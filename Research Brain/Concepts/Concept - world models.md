---
type: concept
id: "concept:world_models"
activation: 0.160
prediction_error: 0.0001
surprise: 0.0000
consolidation_count: 0
tags: [concept]
---

# world models

## Referenced in
- [[Paper - I-JEPA Self-Supervised Learning from Images with a Joint-Embedding Predictive Architecture]]
- [[Paper - A Path Towards Autonomous Machine Intelligence]]


## Connections

### Incoming
- related_to: [[Paper - I-JEPA Self-Supervised Learning from Images with a Joint-Embedding Predictive Architecture]] (w=1.00)
- related_to: [[Paper - A Path Towards Autonomous Machine Intelligence]] (w=1.00)

## Substrate State
- Activation: 0.160
- Prediction Error: 0.0001
- Surprise: 0.0000
- Consolidation Count: 0